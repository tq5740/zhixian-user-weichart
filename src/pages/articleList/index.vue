...
export default {
  ...
  onShow() {
    this.$store.commit("setTitle", "就业动态");
        ...
        Article.article_list(data, (res) => {
          if (res.code === 0) {
            ...
            this.articleListData.forEach(item => {
              item.createTime = getTimeSpan(item.createTime,true)
            });
